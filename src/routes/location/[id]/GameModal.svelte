<script lang="ts">
  ...
  import type { SearchResult, GameDetails } from "$lib/types/boardbuddy-types";
      ...
      try {
          ...
          searchResults = Array.from(boardgames).slice(0, 15).map(boardgame => {
              ...
              return {
                  ...
                  title: titleElement ? titleElement.textContent ?? 'No title' : 'No title',
                  year: yearElement ? yearElement.textContent ?? 'Unknown' : 'Unknown'
              } as SearchResult;
          });
  ...
  async function fetchGameDetails(game: SearchResult): Promise<GameDetails | undefined> {
      try {
  ...
  async function onGameSelect(game: SearchResult) {
      const gameDetails = await fetchGameDetails(game);
      if (gameDetails) {
          const modalResponse = $modalStore[0].response;
          if (typeof modalResponse === 'function') {
              modalResponse(gameDetails);
          }
      }
      modalStore.close();
  }
